...
def test_get_B1B2_basis():
    basis_B_part = numpyqi.group.symext.get_B1B2_basis()
    basis_B = np.concatenate(basis_B_part, axis=0)
    assert np.abs(basis_B @ basis_B.T.conj() - np.eye(len(basis_B))).max() < 1e-7
    for dimA in [2,3,4,5]:
        dm0 = numpyqi.random.rand_ABk_density_matrix(dimA, dimB=3, kext=2)
...
def test_get_B1B2B3_basis():
    # 10 8 8 1
    basis_B_part = numpyqi.group.symext.get_B1B2B3_basis()
    basis_B = np.concatenate(basis_B_part, axis=0)
    assert np.abs(basis_B @ basis_B.T.conj() - np.eye(len(basis_B))).max() < 1e-7
    for dimA in [2,3,4,5]:
        dm0 = numpyqi.random.rand_ABk_density_matrix(dimA, dimB=3, kext=3)
        basis_AB_part = [np.kron(np.eye(dimA),x) for x in basis_B_part]
...
def test_get_sud_symmetric_irrep_basis():
    ...
    for dimB,kext in dimB_kext_list:
        basis_B_list = numpyqi.group.symext.get_sud_symmetric_irrep_basis(dimB, kext)
        ...
        dm0 = numpyqi.random.rand_ABk_density_matrix(dimA, dimB, kext)
...
def test_get_B3_irrep_basis():
    dimA = 2
    for dimB in [2,3,4,5]:
        basis_B_part = numpyqi.group.symext.get_B3_irrep_basis(dimB)
        basis_B = np.concatenate(basis_B_part, axis=0)
        assert np.abs(basis_B @ basis_B.T.conj() - np.eye(len(basis_B))).max() < 1e-7

        dm0 = numpyqi.random.rand_ABk_density_matrix(dimA, dimB, kext=3)
        basis_AB_part = [np.kron(np.eye(dimA),x) for x in basis_B_part]
        basis_AB = np.concatenate(basis_AB_part, axis=0)
        dm1 = basis_AB.conj() @ dm0 @ basis_AB.T
        tmp0 = [(x.conj() @ dm0 @ x.T) for x in basis_AB_part]
...
def test_get_B4_irrep_basis():
    dimA = 2
    for dimB in [2,3,4,5]:
        basis_B_part = numpyqi.group.symext.get_B4_irrep_basis(dimB)
        # 1,3,2,3,1
        basis_B = np.concatenate(basis_B_part, axis=0)
        assert np.abs(basis_B @ basis_B.T.conj() - np.eye(len(basis_B))).max() < 1e-7

        dm0 = numpyqi.random.rand_ABk_density_matrix(dimA, dimB, kext=4)
        basis_AB_part = [np.kron(np.eye(dimA),x) for x in basis_B_part]
        basis_AB = np.concatenate(basis_AB_part, axis=0)
        dm1 = basis_AB.conj() @ dm0 @ basis_AB.T
        tmp0 = [(x.conj() @ dm0 @ x.T) for x in basis_AB_part]

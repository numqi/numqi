...
# for test only
_finite_group_list = [numpyqi.group.get_symmetric_group_cayley_table(n) for n in range(2,6)]
_finite_group_list += [numpyqi.group.get_symmetric_group_cayley_table(n, alternating=True) for n in range(2,6)]
_finite_group_list += [numpyqi.group.get_klein_four_group_cayley_table()]
_finite_group_list += [numpyqi.group.get_dihedral_group_cayley_table(n) for n in range(3,8)]
...
def test_cayley_table_to_left_regular_form():
    for cayley_table in _finite_group_list:
        if len(cayley_table)>20:
            continue
        left_regular_form = numpyqi.group.cayley_table_to_left_regular_form(cayley_table)
...
def test_reduce_group_representation():
    for cayley_table in _finite_group_list:
        ...
        left_regular_form = numpyqi.group.cayley_table_to_left_regular_form(cayley_table) #(np,int,(N0,N0,N0))
        irrep_list = numpyqi.group.reduce_group_representation(left_regular_form) #(list,(np,complex,(N0,N1,N1)))
...
def test_to_unitary_representation():
    ...
    z2 = numpyqi.group.reduce_group_representation(z1)

    tmp0 = numpyqi.group.matrix_block_diagonal(z2[-2], z2[-1])
    tmp1 = hf_randc(*tmp0.shape[1:])
    z3 = tmp1 @ tmp0 @ np.linalg.inv(tmp1)

    tmp0 = numpyqi.group.to_unitary_representation(z3)
...
def test_group_algebra_product():
    for cayley_table in _finite_group_list:
        ...
        index = numpyqi.group.get_index_cayley_table(cayley_table)
        ret0 = numpyqi.group.group_algebra_product(vec0, vec1, index, use_index=True)
...
def test_projection_op():
    for cayley_table in _finite_group_list:
        left_regular_form = numpyqi.group.cayley_table_to_left_regular_form(cayley_table)
        irrep_list = numpyqi.group.reduce_group_representation(left_regular_form)
        op_list = [x.transpose(1,2,0).conj()*(x.shape[1]/x.shape[0]) for x in irrep_list]
        index_product = numpyqi.group.get_index_cayley_table(cayley_table)
        ...
        for ind0 in range(len(irrep_list)):
            ...
            tmp0 = numpyqi.group.group_algebra_product(x0[:,np.newaxis], x0, index_product, use_index=True).reshape([dim_irrep]*4+[dim_group])
            ...
            for ind1 in range(ind0+1,len(op_list)):
                ...
                tmp0 = numpyqi.group.group_algebra_product(x0[:,np.newaxis], x1, index_product, use_index=True)

...
def test_hf_complex_to_real():
    ...
    tmp0 = numpyqi.utils.hf_real_to_complex(numpyqi.utils.hf_complex_to_real(np0))
    assert np.abs(np0-tmp0).max() < 1e-10

    np0 = hf_randc(N0, N1, N1)
    np1 = hf_randc(N0, N1, N1)
    ret_ = np0 @ np1
    tmp0 = [numpyqi.utils.hf_complex_to_real(x) for x in (np0,np1)]
    ret0 = numpyqi.utils.hf_real_to_complex(tmp0[0] @ tmp0[1])

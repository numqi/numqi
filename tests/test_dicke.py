...
def test_dicke_state_partial_trace():
    for n in range(2,8):
        ...
        tmp0 = np.arange(n+1)
        a00,a01,a10,a11 = numqi.dicke.dicke_state_partial_trace(n)
        ...
        for x,y in itertools.product(range(n+1),range(n+1)):
            ...
            tmp0 = numqi.dicke.dicke_state(n, x, return_dm=False)
            tmp1 = numqi.dicke.dicke_state(n, y, return_dm=False)
...
def test_qudit_dicke_state_partial_trace():
    for n,d in itertools.product(range(2, 5), range(2, 5)):
        Bij,klist,klist_to_ij = numqi.dicke.qudit_dicke_state_partial_trace(d, n)
        ...
        for ki,kj in itertools.product(klist, klist):
            ...
            tmp0 = numqi.dicke.qudit_dicke_state(*ki)
            tmp1 = numqi.dicke.qudit_dicke_state(*kj)
...
def test_qudit_partial_trace_AC_to_AB():
    ...
    for dimA in range(2,5):
        for dimB in range(2, 5):
            for k in range(2, 5):
                Bij,klist,klist_to_ij = numqi.dicke.qudit_dicke_state_partial_trace(dimB, k)
                np0 = hf_norm(hf_randc(dimA*len(klist))).reshape(dimA,len(klist))
                ret0 = numqi.dicke.qudit_partial_trace_AC_to_AB(np0, Bij)
                assert abs(np.trace(ret0)-1)<1e-7
                assert np.all(np.linalg.eigvalsh(ret0)+1e-7>0) #almost PSD (ignoring rounding error)

                if torch is not None:
                    Bij_torch = [[torch.tensor(y) for y in x] for x in Bij]
                    ret0 = numqi.dicke.qudit_partial_trace_AC_to_AB(torch.tensor(np0), Bij_torch).numpy()
                    assert abs(np.trace(ret0)-1)<1e-7
                    assert np.all(np.linalg.eigvalsh(ret0)+1e-7>0) #almost PSD (ignoring rounding error)

...
def test_ABkHermitian():
    ...
    for dimA,dimB,kext in itertools.product(dimA_list, dimB_list, kext_list):
        layer = numqi.param.ABkHermitian(dimA, dimB, kext)
        ...
        for ind0,ind1 in tmp0:
            tmp1 = numqi.param._ABk.ABk_permutate(mat, ind0, ind1, dimA, dimB, kext)
...
def test_ABk2localHermitian():
    ...
    kext_list = list(range(1,6))
    for dimA,dimB,kext in itertools.product(dimA_list, dimB_list, kext_list):
        layer = numqi.param.ABk2localHermitian(dimA, dimB, kext)
        ...
        for x in range(1,kext):
            ret_ = ret_ + numqi.param._ABk.ABk_permutate(tmp0, 0, x, dimA, dimB, kext)
...
def test_ABk_2local_symmetry_index():
    ...
    for dimA,dimB,kext in itertools.product(dimA_list, dimB_list, kext_list):
        coeff,index = numqi.param._ABk.ABk_2local_symmetry_index(dimA, dimB, kext)
        tmp0 = np_rng.uniform(-1, 1, size=(dimA*dimB,dimA*dimB))
        np0 = tmp0 + tmp0.T

        parameter = np0[np.triu(np.ones((dimA*dimB,dimA*dimB),dtype=np.bool_))]
        ret0 = numqi.param._ABk.ABk_2local_index_to_full(parameter, coeff, index)
        tmp0 = np.kron(np0, np.eye(dimB**(kext-1)))
        ret_ = tmp0
        for x in range(1,kext):
            ret_ = ret_ + numqi.param._ABk.ABk_permutate(tmp0, 0, x, dimA, dimB, kext)
...
def test_ABk_2local_skew_symmetry_index():
    ...
    np_rng = np.random.default_rng()
    for dimA,dimB,kext in itertools.product(dimA_list, dimB_list, kext_list):
        coeff,index = numqi.param._ABk.ABk_2local_skew_symmetry_index(dimA, dimB, kext)
        tmp0 = np_rng.uniform(-1, 1, size=(dimA*dimB,dimA*dimB))
        np0 = tmp0 - tmp0.T

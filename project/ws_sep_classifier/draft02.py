...
class PositiveWeightLinear(torch.nn.Module):
    def __init__(self, in_features:int, out_features:int, device=None, dtype=torch.float32):
        ...
        self.bias = torch.nn.Parameter(torch.randn(out_features, dtype=dtype, device=device)/np.sqrt(in_features))
...
class DummyConvexModel(torch.nn.Module):
    def __init__(self, dim, dim_fc_list=None, device:(torch.device|None)=None):
        ...
        if dim_fc_list is None:
            dim_fc_list = [dim**2-1, 128, 128, 128, 128, 128, 1]
        else:
...
dimB = 2
batch_size = 2048
num_rho0 = int(0.1*batch_size)
label_rho0 = -10
lr_start_end = 0.03, 0.001
num_step = 10000
step_lr_scheduler = 10

tmp0 = label_rho0*(num_rho0/batch_size)
worst_loss = (label_rho0-tmp0)**2*(num_rho0/batch_size) + tmp0**2*(1-num_rho0/batch_size)
...
torch_rho0 = torch.zeros((num_rho0, dim*dim-1), dtype=torch.float32, device=device)
ydata_i = torch.tensor([label_rho0]*num_rho0+[0]*(batch_size-num_rho0), dtype=torch.float32, device=device)
with tqdm(range(num_step)) as pbar:
    for ind_step in pbar:
        optimizer.zero_grad()
        tmp0 = torch.tensor(generate_ppt_boundary_data(batch_size-num_rho0, (dimA,dimB)), dtype=torch.float32, device=device)
...
np0 = np.linspace(0, np.sqrt((dim-1)/(2*dim)), 100)
with torch.no_grad():

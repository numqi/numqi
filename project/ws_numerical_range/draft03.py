...
# dm0 = pyqet.entangle.load_upb('pyramid', return_bes=True)[1]
# dm1 = pyqet.entangle.get_isotropic_state(dim, 1)
dm1 = pyqet.entangle.load_upb('tiles', return_bes=True)[1]
# dm0 = pyqet.random.rand_density_matrix(dim*dim)
# dm1 = pyqet.random.rand_density_matrix(dim*dim)
...
tmp1 = pyqet.entangle.get_ABk_symmetric_extension_boundary(tmp0, **kwargs)
...
basis0 = pyqet.gellmann.dm_to_gellmann_basis(dm0)
basis1 = pyqet.gellmann.dm_to_gellmann_basis(dm1)
vec_proj_A,vec_proj_N = pyqet.maximum_entropy.get_supporting_plane_2d_projection(vecA_list, vecN_list, basis0, basis1, theta_list)
beta_dm = np.array([pyqet.entangle.get_density_matrix_boundary(hf_theta(x))[1] for x in theta_list])
fig,ax = plt.subplots()
tmp0 = slice(None,None,1)
# pyqet.maximum_entropy.draw_line_list(ax, vec_proj_A[tmp0], vec_proj_N[tmp0], kind='tangent', color='#CCCCCC', radius=0.1, label='SDP dual')
pyqet.maximum_entropy.draw_line_list(ax, vec_proj_A[tmp0], vec_proj_N[tmp0], kind='norm', color='#CCCCCC', radius=0.03, label='SDP dual')

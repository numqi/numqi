...
Brsab = pyqet.dicke.get_partial_trace_ABk_to_AB_index(kext, dimB,  return_tensor=True)

matG = pyqet.gellmann.all_gellmann_matrix(dimA*dimB, with_I=False)
...
model_pureb = pyqet.entangle.PureBosonicExt(dimA, dimB, kext, distance_kind='ree')

theta_optim = pyqet.optimize.minimize(model, num_repeat=1, tol=1e-12, print_freq=10)

for ind_round in range(100):
    loss = pyqet.optimize.minimize_adam(model, num_step=1000, theta0='uniform')
    theta0 = pyqet.optimize.get_model_flat_parameter(model)
    theta_optim = pyqet.optimize.minimize(model, theta0=theta0, num_repeat=1, tol=1e-12, print_freq=40)
    ...
    tmp0 = pyqet.gellmann.gellmann_basis_to_dm(coeff)
    ...
    # ree = pyqet.optimize.minimize(model_pureb, num_repeat=3, tol=1e-10, print_every_round=0).fun
    ree = pyqet.entangle.get_ABk_symmetric_extension_ree(dm_target, (dimA,dimB), kext, use_ppt=False, use_tqdm=True)
...
# beta_ppt = pyqet.entangle.get_ppt_boundary(dm_target, (dimA,dimB))[1]
beta_list = np.linspace(beta_kext-0.01, min(beta_u,beta_kext+0.1), 100)
dm_target_list = [pyqet.entangle.hf_interpolate_dm(tmp0, beta=x) for x in beta_list]

ree_pureb = []
for dm_i in tqdm(dm_target_list):
    model_pureb.set_dm_target(dm_i)
    ree_pureb.append(pyqet.optimize.minimize(model_pureb, num_repeat=3, tol=1e-10, print_every_round=0).fun)
ree_pureb = np.array(ree_pureb)

ree_irrep = pyqet.entangle.get_ABk_symmetric_extension_ree(dm_target_list, (dimA,dimB), kext, use_ppt=False, use_tqdm=True)

...
beta_list,op_nr_list,dual_value_list = numqi.maximum_entropy.op_list_numerical_range_SDP([op0,op1], theta_list)

fig,ax = plt.subplots()
tmp0 = slice(None, None, 3)
tmp1 = np.angle(dual_value_list[:,0] + 1j*dual_value_list[:,1])
# numqi.maximum_entropy.draw_line_list(ax, op_nr_list[tmp0], tmp1[tmp0], kind='norm', color='#CCCCCC', radius=0.06)
numqi.maximum_entropy.draw_line_list(ax, op_nr_list[tmp0], tmp1[tmp0], kind='tangent', color='#CCCCCC', radius=1)
...
data_list = [numqi.maximum_entropy.op_list_numerical_range_SDP([op0,x], theta_list) for x in op1_list]

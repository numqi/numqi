...
def demo_qutrit_coeffB():
    coeffB_list,multiplicity_list = numpyqi.group.symext.get_symmetric_extension_irrep_coeff(dimB=3, kext=2)
    ...
    for ind0,coeffB_i in enumerate(coeffB_list):
        print(f'B^({ind0})')
        numpyqi.group.symext.print_symmetric_extension_irrep_coeffB(coeffB_i)

    coeffB_list,multiplicity_list = numpyqi.group.symext.get_symmetric_extension_irrep_coeff(dimB=3, kext=3)
...
def demo_B3_B4_basis():
    ...
    for dim in [2,3,4,5,6]:
        z0 = numpyqi.group.symext.get_B3_irrep_basis(dim)
    ...
    ind_parameter = ind_parameter.tolist()
    for dim in [2,3,4,5,6]:
        z0 = numpyqi.group.symext.get_B4_irrep_basis(dim)
        tmp0 = [x.shape[0] for x in z0]
        num_parameter = sum([x*x for x,y in zip(tmp0,ind_parameter) if y])
        print(dim, [x.shape[0] for x in z0], num_parameter)

...
def _torch_psd_sqrtm_forward_repeat(matA, repeat=1):
    ...
    repeat = int(repeat)
    assert repeat >= 1
    tag_use_scipy = (not matA.is_cuda) and (matA.ndim==2)
    if tag_use_scipy:
        ret = matA.numpy()
        for _ in range(repeat):
            ret = torch.from_numpy(scipy.linalg.sqrtm(ret).astype(ret.dtype))
            # TODO .astype(matA.dtype) scipy-v1.10 bug https://github.com/scipy/scipy/issues/18250
        ctx_tensor = ret,
    else:
        N0 = matA.shape[-1]
...
def _torch_psd_sqrtm_backward_repeat(grad_output, ctx_tensor, repeat=1):
    ...
    if (not grad_output.is_cuda) and (grad_output.ndim==2):
        ...
        for ind0 in range(repeat):
            ...
            ret = (EVC @ ((EVCh @ ret @ EVC) / tmp0) @ EVCh).view(*shape, N0, N0)
            if ind0!=repeat-1:
                sqrt_EVL = sqrt_EVL**2
    return ret

...
def _matplotlib_latex_context():
    if has_latex:
        ret = matplotlib.rc_context({'text.usetex': True, 'text.latex.preamble': r'\usepackage{amsmath,amsfonts}'})
    else:
        ret = contextlib.nullcontext()
    return ret
...
def hfline_pos(k0, k1, x0, x1, POS, shift=(0,0)):
    ...
    if len(shift)==2: #(x,y):
        ...
        ret0 = ret0[0]+shift[0],ret0[1]+shift[2]
        ret1 = ret1[0]+shift[1],ret1[1]+shift[3]
    return ret0,ret1
...
def plot_qobject_trivialization_map():
...
def plot_cha_trivialization_map():
    POS = {
        'Rn': (-2,0),
        'Sna': (1,1.5),
        'Snb': (1,0),
    ...
    with _matplotlib_latex_context():
        ...
        ax.add_patch(matplotlib.patches.FancyArrowPatch(*hfline('Rn', 'Sna', 0.24, 0.84, shift=(0.3,-0.3,-0.1,0.25)), **arrow_kw))
        ...
        ax.add_patch(matplotlib.patches.FancyArrowPatch(*hfline('Rn', 'Delta', 0.24, 0.80, shift=(0.1,0.1,0,-0.3)), **arrow_kw))
